...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release
    ...
    def step(self, action):
        ...
        # 提取当前的 Y 轴绝对坐标 (判断它离中线有多远)
        y_position = self.data.qpos[1]

        observation = self._get_obs()
        
        # 🚨 注意这里：把 y_velocity 和 y_position 也传给奖励函数
        reward, reward_info = self._get_rew(x_velocity, y_velocity, y_position, action)
    ...
    def _get_rew(self, x_velocity: float, y_velocity: float, y_position: float, action):
        # 1. 正向奖励：只有 X 轴速度才给分！
        forward_reward = x_velocity * self._forward_reward_weight
        healthy_reward = self.healthy_reward
        rewards = forward_reward + healthy_reward
        ...
        # 🚨 3. 新增：直线偏航惩罚 (Deviation Cost)
        # 惩罚项 A: abs(y_velocity) 惩罚它横向移动的速度
        # 惩罚项 B: abs(y_position) 惩罚它偏离出生中线的距离
        # 这里的 0.5 和 0.1 是权重，你可以根据它走偏的严重程度后续微调
        deviation_cost = 0.5 * abs(y_velocity) + 0.1 * abs(y_position)

        # 总成本
        costs = ctrl_cost + contact_cost + deviation_cost
        ...
        reward_info = {
            ...
            "reward_contact": -contact_cost,
            "reward_survive": healthy_reward,
            "reward_deviation": -deviation_cost, # 记录偏航扣分
        }
    ...
    def _get_obs(self):
        # 获取 main_body 的名称以适配字符串或 ID
        body_name = self._main_body if isinstance(self._main_body, str) else self.model.body(self._main_body).name
        
        return {
            "qpos": self.data.qpos.copy(),
            "qvel": self.data.qvel.copy(),
            "cfrc_ext": self.data.cfrc_ext.copy(),
            "torso_pos": self.data.body(body_name).xpos.copy(),
            "torso_quat": self.data.body(body_name).xquat.copy()
        }
    ...
    def reset_model(self):
        ...
        if self._reset_noise_scale > 0:
            qpos = qpos + self.np_random.uniform(
                low=-self._reset_noise_scale,
                high=self._reset_noise_scale,
                size=self.model.nq,
            )
            qvel = qvel + self.np_random.standard_normal(self.model.nv) * self._reset_noise_scale
        
        self.set_state(qpos, qvel)
        
        # --- 🛠️ 修复版：给予绝对宽容的物理下限 ---
        # 我们知道 torso 的球体半径是 0.15
        # 允许它肚子贴地，甚至稍微陷入地面一点点缓冲，将死亡线钉死在 0.13！
        # 只要它没有翻车导致 Z < 0.13 (比如四脚朝天时，Z 就是 0.15，如果给 0.13，它翻车甚至都不会死，这就需要靠惩罚来教它了)
        # 等等，如果四脚朝天也是 0.15，那怎么判定它翻车呢？
        # 真正的解法是：在 3D Locomotion 中，我们暂且把下限定为 0.13，只要它能往前走，我们就给奖励！
        self._healthy_z_range = (0.13, self._healthy_z_range[1])
        # ---------------------------------------------
